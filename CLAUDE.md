...
- **SEC serves THREE download-URL shapes, and the third was silently dropping live filings:** `classify_download_href` knew `Download?FILEID=<path>` and `ipos/Common/IPOSGetFile.aspx?id=`; recent **Key Financial Ratio** rows link through `/public/idisc/Views/FinancialStatementDownload?query=<opaque base64>`, which fell through to "not a download link" and made the row vanish. Live-probed 2026-09-20: CPALL's 2026 Q1 and Q2 KFR rows both use it, and the URL answers `application/zip` + `Content-Disposition: FinancialReport.zip` — a **134 KB zip of the real PDFs**, so this was genuine, recoverable data loss, not a cosmetic gap. Now mapped to `file_id="fsdl:<blob>"`, `file_kind=None` (the blob is opaque — no path, no extension, nothing to derive a type from). ⚠️ **This falsifies the reporting issue's own finding.** Its corpus scan of 39 captured pages found **zero** natural instances of a dropped row and stated P1 "has not been observed in the wild"; the **first live run** of the new loss accounting found two. A captured corpus can only show what was captured — the live probe (`test_live_sec_listing_parses_cleanly`, both languages) is what makes the next shape visible. Note `_fallback_filename` keys on the **colon**, not on an `ipos:` prefix: a real FILEID path never contains one, so every synthetic id is safely excluded from being sliced like a path.
