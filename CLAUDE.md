...
- **`Receive Date` has no Thai mapping — deliberately:** the captured corpus behind the Thai support is FS searches only, so the Thai spelling of the 56-1/56-2 `Receive Date` column was never observed, and a guessed header maps silently to the wrong column if wrong. A Thai 56-1/56-2 listing therefore yields `receive_date=None` while every other field populates. Capture one Thai 56-1 page and add the observed header to fix it.
