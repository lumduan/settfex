...
- **`fs-r561` is an UNMAPPED ViewMore slug — a known, visible 56-1 shortfall (not yet fixed):** `_CATEGORY_FOR_VIEWMORE_SLUG` maps only `fs-norm`/`fs-kf`/`fs-mda`, so a 56-1/56-2 section with more than ~10 records truncates even with `follow_view_more=True`. 0.22.0's CLAUDE.md predicted this and said the shortfall WARNING is what would surface it — **it did**: CPALL's Thai 56-1 reports 12 records and yields 10, and the page carries `ViewMore/fs-r561`, which serves exactly those 12 rows (probed 2026-09-20; the English page has only 6 records so it carries no ViewMore link at all). This is **reported, not silent** — `completeness()` shows `(10, 12)` — so it is a data gap, not a silent-loss bug. Closing it is one entry in the slug map plus whatever `fs-r562` turns out to be.
