...
- **Backward compatibility = 4 levels, all enforced:**
  ...
  - **L4 runtime** — live SET/TFEX/ThaiBMA calls still succeed:
    `uv run pytest -m integration --no-cov`. Set `SETTFEX_LIVE_BUDGET=N` to cap request
    attempts (`tests/integration/conftest.py` refuses attempt N+1; session warmups are not
    counted). The weekly `.github/workflows/live-smoke.yml` runs the same probes on a Thailand
    self-hosted runner, but it is **inert** until a runner labelled `thailand` is registered and
    the repo variable `LIVE_SMOKE_ENABLED` is `true`. Read its header before enabling it: the home
    egress IP is shared with production capture, and the repo is public.
...
- **The analyst-consensus endpoint answers an uncovered symbol with HTTP 500, not 404:** `/api/set-fund/consensus/stock/{sym}/consensus` returns 500 for anything it has no consensus record for — **including valid SET common stocks** (`ABICO`), DRs (`GOOG80`) and warrants (`JAS-W4`). For a **listed** symbol it is therefore raised as a plain `FetchError(status_code=500)`, never `SymbolNotFoundError`: the suggester would emit the absurd "'ABICO' not found — did you mean 'ABICO'?" (the mirror of the DR-profile 404 gotcha above). A genuine server error is indistinguishable, so retry once before concluding a symbol is uncovered. ⚠️ **But a typo gets the same 500, and since 0.25.0 it is told apart:** on a 500, `_is_listed_symbol()` (`list.py`) consults the SET stock list — the in-process one if `get_stock_list()` already ran, otherwise **one** load per event loop (`include_indices=False`, `max_retries=0`; a failure is remembered and answers "unknowable") — and a symbol that is not listed raises `SymbolNotFoundError(status_code=500)` with a real `.suggestion`. Only the per-symbol table is classified; the whole-market summary (which passes `symbol="SET"`) never is. `tests/conftest.py` stubs the lookup to "unknowable" for every unit test, because a test that patches only the consensus fetcher would otherwise send a live SET request.
...
- **The SEC LISTING path checks its HTTP status now — and a ViewMore failure DEGRADES rather than raising (issue #131):** `_run_search` used to return `post_resp.text` without ever reading `status_code`, so an error page parsed to zero rows and came back as `[]` with `has_losses == False` — indistinguishable from an issuer that filed nothing, and intermittent, so a backfill recorded the window as covered. Evidenced by a real **HTTP 505** from the listing host (2026-09-20). Two rules: non-2xx → `FetchError`; a 2xx body with **no result table and no section heading** → `ParseError`. The content rule is deliberately the weakest one that separates the populations, because the case it must never trip is a **genuinely empty listing** — a test pins it against every page this repo holds. It does **not** key on `ctl00_CPH_pnlControl`: the real ViewMore capture has that id and the trimmed `FS_VIEWMORE_HTML` constant does not, so a panel-id rule would call a legitimate page an error. ⚠️ **The severity ladder differs by leg, on purpose.** The POST leg raises (a total loss — nothing to fall back on). A **ViewMore** page is different: it *replaces* its section's inline rows, so 0.22.0's unconditional replacement meant a failing page **deleted the whole section, inline rows included, in silence**. A broken ViewMore is a *partial* loss, so it now keeps the inline rows, warns, and lets `completeness()` show the shortfall. `_map_rows` additionally refuses a **zero-row** input: within this package that can only be a non-listing body, since every captured page carries at least the placeholder row an empty section serves. ⚠️ **U-7 (0.25.0): a ViewMore page must never REDUCE the row set — and until 0.25.0 a *successful* one could.** Two ways, both silent: (1) a page that mapped to fewer documents than the inline rows replaced them anyway (no comparison), so a short or placeholder-only page deleted rows with `has_losses` still `False`; (2) a page whose `_map_rows` raised (`ParseError`/`IncompleteListingError`) escaped the loop, and the per-code gather then failed the **whole report code**, taking every sibling category's inline rows with it. Both now degrade exactly like an unusable page (`DegradedSection`, inline rows kept). The rule is a **count** (ViewMore docs for the category ≥ inline docs), **not identity** — a superset check by file id would flip the synthetic test pairs and misfire on `fsdl:` ids, which may be minted per request. On the degrade path `reported.update`/`supersede` are skipped, so `is_balanced` holds and the inline page's reported count stays for `completeness()`; on the replace path only the replaced category's reported count moves.
